...
Ltog List := Matrix => frmt -> (
    ...
    g' := truncateLieAlgebra(g,0); gMIN := min indices module g';
...
genericGorIndexing (ZZ,List) := Matrix => (n,dvletters) -> (
    ...
    gneg := truncateLieAlgebra(g,0); --0 or -1
    gNeginc := (
...
genericMultigrade (LieAlgebra,Thing) := Matrix => (g) -> (
    if g.cache#?"negativeparametrization" then return g.cache#"negativeparametrization";
    if g.cache#?"parametersymbol" then x := g.cache#"parametersymbol" else x = symbol x;
    kk := ring module g;
    n := numcols g.cache#"e";
    ...
    return g.cache#"negativeparametrization" = sum(keys hgrade, k -> (gens hgrade#k)*(transpose matrix{toList varHT#k}))
    )


genericMultigradeFull = method();
genericMultigradeFull (LieAlgebra) := Matrix => (g) -> (
    if g.cache#?"parametrization" then return g.cache#"parametrization";
    x := symbol x;
    kk := ring module g;
    n := numcols g.cache#"e";
    grd := apply(n, i -> (bracket g)*(g.cache#"h"_{i}**id_(module g)));
    ...
    for i from 0 to n-1 do (
	    ...
	    for j from -degmax_i to degmax_i do (
	    	temp := intersect (hgrade#k, ker (grd#i+j));
	    	if not zero temp then newhgrade#(k|{j}) = temp
    	    	);
	    );
    ...
    g.cache#"parametersymbol" = x;
    return g.cache#"parametrization" = sum(keys hgrade, k -> (gens hgrade#k)*(transpose matrix{toList varHT#k}))
    )
...
topComplex (ZZ,List,Matrix) := HashTable => (s,frmt,subm) -> (
    ...
    g := lieAlgebra algname; n := algname_1; g' := truncateLieAlgebra(g,0); F := kk^n;
    slF := slLie(F);
    ------------------------------------
