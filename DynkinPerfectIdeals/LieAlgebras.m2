...
lieAlgebra (String,ZZ,Ring) := Thing => (fam,n,kk) -> (
    ...
    e := new MutableHashTable; f := new MutableHashTable; coroots := new MutableHashTable;
    ...
    coroots#0 = (bracket g)*(e#0**f#0);
    --eall := e#0|epos; fall := f#0|fpos; corootsall := fold(apply(toList(0..(n-1)),i->coroots#i),(a,b)->a|b);
    if fam=="E" then (
    ...
    g.cache#"coroots" = fold(apply(bourbaki,i->coroots#i),(a,b)->a|b);
    CartanMatrix := fold(apply(n,i->((bracket g)*(g.cache#"coroots"**g.cache#"e"_{i}))//g.cache#"e"_{i}), (a,b)->a||b);
    --dual to simple roots
...
extremalRep = method(); --this should be renamed to negRep or something later
...
extremalRepFull = method(); --this should be renamed to extremalRep
extremalRepFull(LieAlgebra,ZZ) := LieAlgebraRep => (g,m) -> (
    if not g.cache#?"extremalreps" then g.cache#"extremalreps" = new MutableHashTable;
    if g.cache#"extremalreps"#?m then return g.cache#"extremalreps"#m;
    if member(((name g),m),{(("E",6),2),(("E",7),1),(("E",8),8)}) then return g.cache#"extremalreps"#m = adjointRep g;
    nV := extremalRep(g,m);
    return g.cache#"extremalreps"#m = repFromGradedAction(
	g,
	completeGradedAction(
	    g,
	    completeIrrepAction(g, actionHT nV, module nV),
	    module nV),
	module nV)
    );
...
Dnrep (ZZ,ZZ) := (LieAlgebra,HashTable,Module) => (n,m) -> (
    gneg := truncateLieAlgebra(lieAlgebra("D",n),0);
    kk := QQ; F := kk^n;
    if m==1 then (
	V := directSum{
	    0=>dual F,
...
E6rep (ZZ) := (LieAlgebra,HashTable,Module) => (m) -> (
    gneg := truncateLieAlgebra(lieAlgebra("E",6),0);
    kk := QQ; F := kk^6;
    if m==6 then (
	V := directSum{
    	    0=>F,
...
E7rep (ZZ) := (LieAlgebra,HashTable,Module) => (m) -> (
    gneg := truncateLieAlgebra(lieAlgebra("E",7),0);
    kk := QQ; F := kk^7;
    if m==7 then (
	V := directSum{
	    0=>dual F,
