...
--truncation
truncateLieAlgebra = method()
--the part >= m
--generally only makes sense if m>=0
truncateLieAlgebra(ZZ,LieAlgebra) := LieAlgebra => (m,g) -> (
    MAX := max indices module g;
    return subAlgebra(g_(new Array from m..MAX),g)
    );
--the part <= m
--generally only makes sense if m<=0
truncateLieAlgebra(LieAlgebra,ZZ) := LieAlgebra => (g,m) -> (
    MIN := min indices module g;
    return subAlgebra(g_(new Array from MIN..m),g)
    );

LieAlgebra++LieAlgebra := LieAlgebra => (g1,g2) -> (
    modg := (module g1)++(module g2);
    actM := (
	modg_[0]*(bracket g1)*(modg^[0]**modg^[0])
	+modg_[1]*(bracket g2)*(modg^[1]**modg^[1])
	);
    g := newLieAlgebra(modg,actM);
    g.cache#"e" = modg_[0]*g1.cache#"e"|modg_[1]*g2.cache#"e";
    g.cache#"f" = modg_[0]*g1.cache#"f"|modg_[1]*g2.cache#"f";
    g.cache#"h" = modg_[0]*g1.cache#"h"|modg_[1]*g2.cache#"h";
    g.cache#"coroots" = modg_[0]*g1.cache#"coroots"|modg_[1]*g2.cache#"coroots";
    g.cache#"CM" = directSum(0 => g1.cache#"CM", 1 => g2.cache#"CM");
    return g
    )
...
LieAlgebraRep++LieAlgebraRep := LieAlgebraRep => (V1,V2) -> (
    g := algebra V1;
    modV := (module V1)++(module V2);
    actM := (
	modV_[0]*(action V1)*(id_(module g)**modV^[0])
	+modV_[1]*(action V2)*(id_(module g)**modV^[1])
	);
    return newRep(g,actM,modV)
    )
...
slLie Module := LieAlgebra => F -> (
    ...
    if n<=1 then (
	g.cache#"e" = g.cache#"f" = g.cache#"h" = g.cache#"coroots" = map(module g,kk^0,0);
    g.cache#"CM" = map(ZZ^0,ZZ^0,0);
	) else (
    	g.cache#"e" = fold(
	    apply(n-1,
	...
	g.cache#"coroots" = fold(
	    apply(n-1,
		i -> (bracket g)*(g.cache#"e"_{i}**g.cache#"f"_{i})
		), (a,b)->a|b
	    );
	CartanMatrix := fold(apply(n-1,i->((bracket g)*(g.cache#"coroots"**g.cache#"e"_{i}))//g.cache#"e"_{i}), (a,b)->a||b);
    	--dual basis to simple roots
    	g.cache#"h" = (g.cache#"coroots"*(inverse CartanMatrix));
        g.cache#"CM" = lift(CartanMatrix,ZZ);
        );
    return g
    )
...
soLie Module := LieAlgebra => M -> (
    ...
    gl := glLie(MM);
    coroots := fold(apply(n, i -> (bracket gl)*(ee_{i}**ff_{i})), (a,b)->a|b);
    soinc := generate(ee|ff,gl);
    so := subAlgebra(soinc,gl);
    so.cache#"e" = ee//soinc; so.cache#"f" = ff//soinc; so.cache#"coroots" = coroots//soinc;
    CartanMatrix := fold(apply(n,i->((bracket so)*(so.cache#"coroots"**so.cache#"e"_{i}))//so.cache#"e"_{i}), (a,b)->a||b);
    --dual basis to simple roots
    so.cache#"h" = (so.cache#"coroots"*(inverse CartanMatrix));
    so.cache#"gl" = soinc;
    so.cache#"CM" = lift(CartanMatrix,ZZ);
    return so
    )
...
slStandard = method()
slStandard Module := LieAlgebraRep => F -> (
    return subRep(slLie(F),isl(F),id_(F),glStandard(F));
    )
...
--find the weight space for a given weight
--can then use "generate" to compute Demazure module
weightSpace = method();
weightSpace(LieAlgebraRep,Matrix) := Matrix => (V,lambda) -> (
    g := algebra V;
    rootcoeffs := lambda*(inverse (QQ**g.cache#"CM")); n := numcols rootcoeffs;
    return gens trim intersect(apply(1..n,
    i -> ker((action V)*(g.cache#"h"_{i-1}**id_(module V))-rootcoeffs_(0,i-1))
    ))
    )
